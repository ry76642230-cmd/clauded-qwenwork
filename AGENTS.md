...
├── bridge-shim-wrapper.sh   # macOS shell wrapper（绕过 PATH 限制）
├── bridge-shim.test.mjs     # 自动化测试（模拟 SDK 调用 shim）
├── index.mjs                # pnpm apply / unapply 注册工具（跨平台）
...
- macOS 平台特殊：SDK spawn 时 PATH 被限制为系统路径，需用 shell wrapper 探测 node 绝对路径
